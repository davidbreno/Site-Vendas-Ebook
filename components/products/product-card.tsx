      ...
      <div className="relative h-56 bg-gradient-to-br from-cream/60 to-white p-6 flex items-center justify-center">
        {product.imageUrl && !imageError ? (
          <img
            ...
            alt={product.title}
            className={`relative z-10 max-h-full w-full object-contain transition-opacity duration-300 ${
        ...
        <span className="absolute top-4 right-4 z-20 bg-white/80 text-navy/70 text-xs px-3 py-1 rounded-full border border-gold/20">
          {product.category?.name ?? 'Produto'}
        </span>

        {(!product.imageUrl || imageError || !imageLoaded) && (
          <div className="absolute z-10 bottom-4 left-1/2 -translate-x-1/2 w-32 h-40 bg-gradient-to-br from-gold/20 to-gold/5 border border-gold/30 rounded-lg shadow-2xl transform group-hover:scale-105 group-hover:-translate-y-2 transition-transform duration-500">
